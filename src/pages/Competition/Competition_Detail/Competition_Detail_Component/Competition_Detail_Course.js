            ...
            <Box sx={{display:'flex',justifyContent:'start',alignItems:'center',width:'100%',mt:'11px',ml:'-2px'}}>
                <Box sx={{px:'20px',display:"flex"}}>
                {
                    props.competition.courseTags.map((item,index)=>{
                        return(
                            <Box key = {index} onClick ={()=>handleToggle(index)} backgroundColor={current===index?'primary.main':''}  sx={{border:1,borderRadius:'8px',p:'4px 8px 4px 8px',display:'flex',justifyContent:'center',alignItems:'center',mr:'6px',borderColor:'#E8E8E8'}}>
